...
export function JournalsTab({ analysis }: { analysis: AnalysisResult }) {
  ...
  const rows = useMemo(() => query.data?.map((row) => ({
    ...row,
    ...(analysis.jifBySourceId.get(row.sourceId) ? { jif: analysis.jifBySourceId.get(row.sourceId) } : {}),
  })), [query.data, analysis.jifBySourceId]);
  return (
    <div id="panel-journals" role="tabpanel" aria-labelledby="tab-journals" className="tab-panel">
      <section className="data-section" aria-labelledby="journals-heading">
        <div className="section-heading-row">
          ...
          {rows ? <button type="button" className="secondary-button" onClick={() => downloadJournalsCsv(analysis, rows)}><Download size={16} /> Download CSV</button> : null}
        </div>
